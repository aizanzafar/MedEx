...
def transform_triple_to_hrt(triple,vocab_dic):
    ...
    # print([vocab_dic[h], vocab_dic[r], vocab_dic[t]])
    return [vocab_dic[h], vocab_dic[r], vocab_dic[t]]

def convert_tokens_to_id(kg,vocab_dic):
    kg = parse_triple(kg)
...
#     for ti in kg:
#         # print(ti)
#         # print([vocab_dic[ti[0]], vocab_dic[ti[1]], vocab_dic[ti[2]]])
#         triples_index.append([vocab_dic[ti[0]], vocab_dic[ti[1]], vocab_dic[ti[2]]])
#     return triples_index

def squad_convert_example_to_features(example, tokenizer, max_seq_length, doc_stride, max_query_length, padding_strategy, is_training):
    ...
    if tokenizer_type == 'roberta':
        ...
        # # print("relation_dic loaded")
        # n_voc = open(vocab_path).read().split('\n')[:-1]

        # # print("new vocab file loaded")
        # vocab_dic = {}
    ...
    for (i, token) in enumerate(example.doc_tokens):
        ...
        if tokenizer.__class__.__name__ in [
            ...
            # print("tokenizer no need to add_prefix_space")
    ...
    while len(spans) * doc_stride < len(all_doc_tokens):
        ...
        if tokenizer.padding_side == "right":
            # print("tokenizer.padding_side is right")
            texts = truncated_query
            pairs = span_doc_tokens
            truncation = TruncationStrategy.ONLY_SECOND.value
        else:
            # print("tokenizer.padding_side is left")
    ...
    for span in spans:
        ...
        kg_enc = example.kg_input[:20]
        kg_enc_input = convert_tokens_to_id(kg_enc,vocab_dic)

        # print("!!!!!!!!!!!!!! check this feature !!!!!!")
        # print(span["input_ids"], type(span["input_ids"]),len(span["input_ids"])) ## list 512
        # print(kg_enc_input, type(kg_enc_input),len(kg_enc_input)) ## list 20
...
def squad_convert_examples_to_features(
    ...
    # print("##### Convert to Tensors and build dataset#####", len(features))
    # for i,f in enumerate(features):
    #     print(i)
    #     print(f.input_ids)
    #     print(f.kg_input)
    #     print(len(f.input_ids))
    #     print(len(f.kg_input))
    ...
    # print("^^^^^^^^^^^^^^^^^^ all_kg_inputs ^^^^^^^^^^^^^")
    # print(all_kg_inputs)
    # print(type(all_kg_inputs))
    # print(len(all_kg_inputs))
    if not is_training:
