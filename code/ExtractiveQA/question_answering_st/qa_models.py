...
from question_answering_st.config.model_args import QuestionAnsweringArgs
from question_answering_st.qa_utils import (
...
# from question_answering_st.bert_qa_model import QAModel_Bert
# from question_answering_st.bert_qa_model import bert_model
from question_answering_st.roberta_qa_model import QA_model
...
class QuestionAnsweringModel:
    def __init__(
        ...
        MODEL_CLASSES = {
            ...
            "roberta": (RobertaConfig, QA_model, RobertaTokenizer),
        ...
        print("!!!!!!!!! qa_models.py  ############  ",model_class, model_name, model_type,"  ############  !!!!!!!!!")
        self.config = config_class.from_pretrained(model_name, **self.args.config)
        if not self.args.quantized_model:
            print("model 1")
            # if model_type =='bert':
            #     rel_dic="vocab/bert/pharmKG_relation_dic.pkl"
            #     vocab_path="vocab/bert/pharmKG_vocab.txt"

            #     relation_dic = pickle.load(open(rel_dic,'rb'))
            #     print("bert relation_dic loaded in question_answering_model")
            #     n_voc = open(vocab_path).read().split('\n')[:-1]

            #     print("new bert vocab file loaded in question_answering_model")
            #     vocab_dic = {}
            #     for i,item in enumerate(n_voc):
            #         vocab_dic[item] = i
            if model_type =='roberta':
                # rel_dic="vocab/roberta/pharmKG_relation_dic.pkl"
                vocab_path="vocab/roberta/entity_vocab_dic.pkl"
                vocab_dic = pickle.load(open(vocab_path,'rb'))

                # relation_dic = pickle.load(open(rel_dic,'rb'))
                print("roberta vocab_dic loaded in question_answering_model")
                # n_voc = open(vocab_path).read().split('\n')[:-1]

                # print("new roberta vocab file loaded in question_answering_model")
                # vocab_dic = {}
                # for i,item in enumerate(n_voc):
                #     vocab_dic[item] = i
                self.model = model_class(model_name, self.config.hidden_size, len(vocab_dic), 300)
            else:
    ...
    def load_and_cache_examples(
        ...
        # cached_features_file = os.path.join('../KG/features/roberta_feature_mean/',"{}_1000".format(mode))
        ...
        if os.path.exists(cached_features_file) and (
            ...
            all_kg_inputs = torch.tensor([f.kg_input for f in features], dtype=torch.long)
        ...
        **kwargs,
        ...
        for _ in train_iterator:
            ...
            for step, batch in enumerate(batch_iterator):
                ...
                # print("!!!!!!!!! ",type(loss),loss) # <class 'torch.Tensor'> tensor(5.8508, device='cuda:0', grad_fn=<DivBackward0>)
                current_loss = loss.item()
                # print("!!!!!!!!! current_loss: ",type(current_loss),current_loss) #  <class 'float'> 5.850830078125
